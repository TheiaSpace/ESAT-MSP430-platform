...
void RealTimeClock::setCalibration(int8_t calibrationValue)
{
	...
	if (calibrationValue<=0) //if value is negative, a decrement is desired (if it is zero, calibration is disabled)
	{
		calibrationValue*=-1; //change to positive, disable positive sign bit, mask and divide by two because each step is -2 ppm
		RTCCTL23_L = (RTCCAL5_L | RTCCAL4_L |RTCCAL3_L | RTCCAL2_L | RTCCAL1_L | RTCCAL0_L) & (((uint8_t)calibrationValue) / 2);
	}
	else
	{
		//enable positive bit, mask and divide by 4 because each step is +4 ppm
		RTCCTL23_L = RTCCALS_L | ((RTCCAL5_L | RTCCAL4_L | RTCCAL3_L | RTCCAL2_L | RTCCAL1_L | RTCCAL0_L) & (((uint8_t)calibrationValue) / 4));
	}
	return;
}
...
{
	if ((frequency == RTC_CALIBRATION_SIGNAL_512HZ) || (frequency == RTC_CALIBRATION_SIGNAL_256HZ) || (frequency == RTC_CALIBRATION_SIGNAL_1HZ ))
	{
